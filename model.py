...
from libs.img_gcn import IM_GCN
from libs.text_atten import TextAttn
...
class EncoderImage(nn.Module):
    ...
    def __init__(self, img_dim, embed_size, no_imgnorm=False):
        ...
        self.IM_GCN_1 = IM_GCN(in_channels=embed_size, inter_channels=embed_size)
        self.IM_GCN_2 = IM_GCN(in_channels=embed_size, inter_channels=embed_size)
        self.IM_GCN_3 = IM_GCN(in_channels=embed_size, inter_channels=embed_size)
        self.IM_GCN_4 = IM_GCN(in_channels=embed_size, inter_channels=embed_size)
        self.init_weights()
    ...
    def forward(self, images):
        ...
        GCN_img_emd = img_emb.permute(0, 2, 1)
        GCN_img_emd = self.IM_GCN_1(GCN_img_emd)
        GCN_img_emd = self.IM_GCN_2(GCN_img_emd)
        GCN_img_emd = self.IM_GCN_3(GCN_img_emd)
        GCN_img_emd = self.IM_GCN_4(GCN_img_emd)
...
class EncoderText(nn.Module):
    ...
    def __init__(self, vocab_size, word_dim, embed_size, num_layers, drop_out,
        ...
        self.W_att1 = TextAttn(word_dim, drop_out)
        self.W_att2 = TextAttn(word_dim, drop_out)
        self.W_att3 = TextAttn(word_dim, drop_out)
        self.W_att4 = TextAttn(word_dim, drop_out)
    ...
    def forward(self, captions, lengths):
        ...
        att_word = self.W_att1(cap_emb,cap_emb)
        att_word = self.W_att2(att_word,att_word)
        att_word = self.W_att3(att_word,att_word)
        att_word = self.W_att4(att_word,att_word)
        cap_emb = att_word
        # pack the caption
...
class EncoderSimilarity(nn.Module):
    ...
    def __init__(self, embed_size, sim_dim, module_name='AVE', sgr_step=3):
        ...
        self.cal_sim_base_atten = SimilarityCalculation(sim_dim)
    ...
    def forward(self, img_emb, cap_emb, cap_lens):
        ...
        for i in range(n_caption):
            ...
            sim_vec = self.cal_sim_base_atten(sim_emb)
...
class LHSC(object):
    def __init__(self, opt):
        ...
        self.img_enc = EncoderImage(opt.img_dim, 
                                    opt.embed_size,
        ...
        self.sim_enc = EncoderSimilarity(opt.embed_size, 
                                         opt.sim_dim)
    ...
    def forward_loss(self, sims, **kwargs):
        ...
        self.logger.update('a/Loss', loss.item(), sims.size(0))
        return loss
