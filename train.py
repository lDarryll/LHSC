"""
# Pytorch implementation for ICMR 22 paper from
# https://doi.org/10.1145/3512527.35313588.
...
def validate(opt, val_loader, model):
    ...
    logging.info("rsum: %.1f" % r_sum)


    # record metrics in tensorboard
    tb_logger.log_value('a/r1', r1, step=model.Eiters)
    tb_logger.log_value('a/r5', r5, step=model.Eiters)
    tb_logger.log_value('a/r10', r10, step=model.Eiters)
    tb_logger.log_value('b/medr', medr, step=model.Eiters)
    tb_logger.log_value('b/meanr', meanr, step=model.Eiters)
    tb_logger.log_value('a/r1i', r1i, step=model.Eiters)
    tb_logger.log_value('a/r5i', r5i, step=model.Eiters)
    tb_logger.log_value('a/r10i', r10i, step=model.Eiters)
    tb_logger.log_value('b/medri', medri, step=model.Eiters)
    tb_logger.log_value('b/meanr', meanr, step=model.Eiters)
    tb_logger.log_value('a/r_sum', r_sum, step=model.Eiters)
